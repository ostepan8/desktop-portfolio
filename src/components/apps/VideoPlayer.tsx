...
/**
 * QuickTime-style video player: playlist sidebar + native <video> element.
 * Content comes from src/constants/videos.ts (files in /public/videos).
 */
export function VideoPlayer({ initialVideoId }: VideoPlayerProps) {
  ...
  return (
    <div className="h-full flex flex-col sm:flex-row bg-black">
      {/* Playlist */}
      <div className="sm:w-56 shrink-0 sm:h-full overflow-auto bg-[var(--macos-bg)] border-b sm:border-b-0 sm:border-r border-white/10">
        <div className="px-3 py-2 text-[11px] uppercase tracking-wider text-white/40 font-medium">
          Playlist
        </div>
        {VIDEOS.map((video) => (
          <button
